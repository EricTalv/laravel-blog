      ...
      <div class="col-12">
        <div class="alert alert-success" role="alert" v-if="createdArticle">
          <h4 class="alert-heading">
            Article <b>"{{ createdArticle.title }}"</b> Created!
          </h4>
          <p>You have successfully made an article.</p>
          <hr />
          <a target="_blank" v-bind:href="createdArticle.path"
            >Check it out here</a
          >
        </div>
        <div class="alert alert-success" role="alert" v-if="updatedArticle">
          <h4 class="alert-heading">
            Article <b>"{{ updatedArticle.title }}"</b> Updated!
          </h4>
          <p>You have successfully Updated the article.</p>
          <hr />
          <a target="_blank" v-bind:href="updatedArticle.path"
            >Check it out here</a
          >
        </div>
      </div>
    ...
    <div class="row">
      <div class="col-6">
        ...
        <form @submit.prevent="submit" novalidate>
          <div
            class="form-group"
            :class="{ 'form-group--error': $v.fields.title.$error }"
          >
            ...
            <div
              class="error"
              v-if="$v.fields.title.$dirty && !$v.fields.title.required"
            >
              <small>Title is required</small>
            </div>
            <div class="error" v-if="!$v.fields.title.maxLength">
              <small
                >Title can only have
                {{ $v.fields.title.$params.maxLength.max }} letters.</small
              >
            </div>
          </div>
          ...
          <div
            class="form-group"
            :class="{ 'form-group--error': $v.fields.excerpt.$error }"
          >
            <label for="excerpt">
              <h3>Excerpt</h3>
            </label>
            ...
            <div
              class="error"
              v-if="$v.fields.excerpt.$dirty && !$v.fields.excerpt.required"
            >
              <small>Excerpt is required</small>
            </div>
            <div class="error" v-if="!$v.fields.excerpt.maxLength">
              <small
                >Excerpt can only have
                {{ $v.fields.excerpt.$params.maxLength.max }} letters.</small
              >
            </div>
          </div>
          <div
            class="form-group"
            :class="{ 'form-group--error': $v.fields.body.$error }"
          >
            ...
            <div
              class="error"
              v-if="$v.fields.body.$dirty && $v.fields.body.$error"
            >
          ...
          <p class="text-success my-2" v-if="submitStatus === 'OK'">
            Thanks for your submission!
          </p>
          <p class="text-danger my-2" v-if="submitStatus === 'ERROR'">
            Please fill the form correctly.
          </p>
          <p class="text-warning my-2" v-if="submitStatus === 'PENDING'">
            Sending...
          </p>
      ...
      <div class="col-6">
        ...
        <div class="bg-white p-2 rounded border border-light">
          <div class="title">
            <div class="clearfix">
              <small class="text-muted float-left text-capitalize">{{
                this.$userName
              }}</small>
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      if (this.$v.$invalid) {
        ...
        if (this.editData) {
          axios
            .put("/articles/" + this.editData.id, this.fields)
            .then((response) => {
          ...
          axios
            .put("/article/create", this.fields)
            .then((response) => {
              this.submitStatus = "SUCCESS";
  ...
  created: function () {
    if (this.editData) {
      ...
      this.articleDateTime.date = moment(this.editData.created_at).format(
        "D MMM, YYYY"
      );
    } else {
